...
%% esCareta/1
esCareta(Festival) :-
	festival(Festival, _, BandasQueTocan, _),
	estaDeModa(UnaBanda),
	estaDeModa(OtraBanda),
	UnaBanda \= OtraBanda,
	member(UnaBanda, BandasQueTocan),
	member(OtraBanda, BandasQueTocan).

esCareta(Festival) :-
	festival(Festival, _, BandasQueTocan, _),
	member(miranda, BandasQueTocan).

%% entradaRazonable/2
entradaRazonable(Festival, Entrada) :-
	.

precioEntrada(Festival, campo, PrecioBase) :-
	festival(Festival, _, _, PrecioBase).

precioEntrada(Festival, plateaNumerada(NumeroDeFila), Precio) :-
	festival(Festival, _, _, PrecioBase),
